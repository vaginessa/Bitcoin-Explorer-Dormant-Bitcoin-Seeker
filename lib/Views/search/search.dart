            ...
            child: TabBarView(
              children: [
                ...
                if (WalletGeneratorState.searchResultByAddress != null)
                  Column(
                      children: [WalletGeneratorState.searchResultByAddress!])
                else if (isSearching)
                  const SizedBox(
                      width: 20,
                      height: 20,
                  ...
                  Center(
                      child: Text(
                    onFirst ? "Empty" : "Invalid address",
                    style: const TextStyle(color: Colors.white, fontSize: 22.5),
                  )),
                // TAB PRIVATE KEY
                if (WalletGeneratorState.searchResultByPrivateKey != null)
                  ...
                  Center(
                      child: Text(
                    onFirst ? "Empty" : "Invalid private key",
                    style: const TextStyle(color: Colors.white, fontSize: 22.5),
                  )),
                // TAB SEED PHRASE
  ...
  void onSearch(String search) async {
    ...
    if (currentTabIndex == 0) {
      ...
      if (params["privateKey"] == "") {
        ...
        try {
          searchThread =
              await Isolate.spawn(bitcoin.searchByPrivateKey, params);
        } on Exception {
          setState(() {
            isSearching = false;
          });
...
class TabButton extends StatelessWidget {
  const TabButton({Key? key, required this.title}) : super(key: key);

  final String title;

  @override
  Widget build(BuildContext context) {
    return SizedBox(height: 45, child: Center(child: Text(title)));
  }
}
