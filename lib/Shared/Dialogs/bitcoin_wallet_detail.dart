...
class _BitcoinWalletDetailState extends State<BitcoinWalletDetail> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title:
            const Text("Bitcoin Wallet", style: TextStyle(color: Colors.white)),
        ...
        child: Column(
          ...
          children: [
            Wrap(
              children: [
                Text(
                  widget.wallet.balance.toStringAsFixed(2) + " BTC",
                ...
                if (widget.wallet.seed != null)
                  KeyBanner(
                    title: "12 phrases seed",
                    content: widget.wallet.seed!,
                    onSeed: true,
                  ),
                if (widget.wallet.privateKey != null)
                  KeyBanner(
                    title: "Private key",
                    content: widget.wallet.privateKey!,
                    onSeed: false,
                  ),
                if (widget.wallet.publicKey != null)
                  KeyBanner(
                    title: "Public key",
                    content: widget.wallet.publicKey!,
                    onSeed: false,
                  ),
                KeyBanner(
                  title: "Address",
                  content: widget.wallet.address,
                  onSeed: false,
                ),
              ],
            ),
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () async {
                  String url = "https://www.blockchain.com/btc/address/" +
        ...
        GestureDetector(
          child: Container(
            ...
            child: Column(
                ...
                children: [
                  Text(
                    content != "" || onSeed ? content : "Unknown",
                    style: const TextStyle(
          ...
          onTap: () {
            Clipboard.setData(ClipboardData(
                    text: content != "" || onSeed ? content : "Unknown"))
                .then((_) {
                    ...
                    .showSnackBar(SnackBar(
                        content: Text(
                            (content != "" || onSeed ? content : "Unknown") +
                                " copied to clipboard")))
                    .closed
                    .then((value) => {canShowClipboardAlert = true});
